/**
 * Node-side OpenClaw web search wrapper.
 * Port of /Users/feiyang/all_bots/om-world/lib/openclaw_web.ts.
 *
 * Compute nodes that opt into `research.cite_synthesis` need their own
 * OpenClaw install (same dep as the LLM path). DuckDuckGo, no API key.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      try {
        ...
        reject(new Error(`openclaw web search JSON parse failed: ${message}`));
      }
    });
  });
}
...
export async function multiSearch(opts: {
  ...
  for (let i = 0; i < opts.queries.length; i++) {
    ...
    try {
      ...
      console.warn(`[node:web] query "${q.slice(0, 60)}" failed:`, e instanceof Error ? e.message : e);
      continue;
    }
    for (const h of hits) {
      if (out.length >= maxTotal) break;
